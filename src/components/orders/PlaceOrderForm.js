...
import {Button, Form, Icon, Input, Popover, Slider, Modal} from 'antd';
import intl from 'react-intl-universal';
import config from 'common/config'
import * as fm from 'LoopringJS/common/formatter'
import * as orderFormatter from 'modules/orders/formatters'
import * as tokenFormatter from 'modules/tokens/TokenFm'
import {getMarketPrice} from "../../modules/orders/formatters"
...
class PlaceOrderForm extends React.Component {
  ...
  componentWillReceiveProps(newProps) {
    if (this.props.marketcap.items !== newProps.marketcap.items && newProps.marketcap.items.length > 0 && this.state.initial) {
      const {marketcap} = newProps;
      ...
      if (this.state.initial) {
        dispatch({type: 'placeOrder/priceChange', payload: {priceInput: currentPrice.toString()}})
      }
    }
  }
  ...
  render() {
    ...
    const amountSliderField = form.getFieldDecorator('amountSlider', {
      ...
      <Slider className="place-order-amount-percentage" min={0} max={100} marks={marks}
              ...
              tipFormatter={null}
              disabled={placeOrder.side === 'sell' ? fm.toBig(sell.availableAmount).lt(0) : fm.toBig(buy.availableAmount).lt(0)}/>
    )

    const totalDisplay = (
      <span>
        {marketConfig && total && fm.toBig(total).gt(0) ? FormatAmount({
          value: total.toString(10),
          precision: marketConfig.pricePrecision
        }) : 0}
      </span>
    )
    ...
    const totalWorthDisplay = (
      <span className="">
        ...
        <span>≈ {fm.getDisplaySymbol(settings.preference.currency)}
          {FormatAmount({
            value: orderFormatter.calculateWorthInLegalCurrency(marketcap.items, right.symbol, total).toFixed(2),
            precision: 2
          })}
          </span>
        }
    ...
    if (ttl.timeToLivePatternSelect === 'easy') {
      ...
      switch (unit) {
        case 'minute':
          ttlInSecond = ttlNumber * 60;
          ttlShow = `${ttlNumber} ${intl.get('common.minute')}`;
          break;
        case 'hour':
          ttlInSecond = ttlNumber * 3600;
          ttlShow = `${ttlNumber} ${intl.get('common.hour')}`;
          break;
        case 'day':
          ttlInSecond = ttlNumber * 86400;
          ttlShow = `${ttlNumber} ${intl.get('common.day')}`;
          break;
        case 'week':
          ttlInSecond = ttlNumber * 7 * 86400;
          ttlShow = `${ttlNumber} ${intl.get('common.week')}`;
          break;
        case 'month':
          ttlInSecond = ttlNumber * 30 * 86400;
          ttlShow = `${ttlNumber} ${intl.get('common.month')}`;
          break;
      }
    } else {
      ...
      <div>
        <div className="card-body form-dark">
          ...
          <ul className="token-tab">
            <li className="buy active"><a data-toggle="tab"
                                          onClick={sideChange.bind(this, 'buy')}>{intl.get('common.buy')} {left.symbol}</a>
            </li>
            <li className="sell"><a data-toggle="tab"
                                    onClick={sideChange.bind(this, 'sell')}>{intl.get('common.sell')} {left.symbol}</a>
            </li>
          </ul>
          }
          {placeOrder.side === 'sell' &&
          <ul className="token-tab">
            <li className="buy"><a data-toggle="tab"
                                   onClick={sideChange.bind(this, 'buy')}>{intl.get('common.buy')} {left.symbol}</a>
            </li>
            <li className="sell active"><a data-toggle="tab"
                                           onClick={sideChange.bind(this, 'sell')}>{intl.get('common.sell')} {left.symbol}</a>
            </li>
          </ul>
          }
          <div className="tab-content">
            ...
            <div className="" id="b1">
              ...
              <div className="pt5 pb5" style={{border: '0px solid rgba(255,255,255,0.07)', margin: '0px 0px'}}>
                <MenuItem label={intl.get('common.total')}
                          value={<div>{totalDisplay} {right.symbol} {totalWorthDisplay}</div>}/>
                <MenuItem label={intl.get('common.lrc_fee')}
                          action={<div onClick={setLRCFee} className="cursor-pointer">{lrcFeeValue} LRC <Icon
                            type="right" className=""/></div>}/>
                <MenuItem label={intl.get('common.ttl')}
                          action={<div onClick={setTTL} className="cursor-pointer">{ttlShow} <Icon type="right"
                                                                                                   className=""/>
                          </div>}/>
              </div>
              ...
              {
                side === 'buy' &&
                <Button className="btn btn-block btn-success btn-xlg" onClick={handleSubmit.bind(this, 'market_order')}
                        loading={placeOrder.submitButtonLoading}>{intl.get('actions.place_buy_order')}</Button>
              }
              {
                side === 'sell' &&
                <Button className="btn btn-block btn-danger btn-xlg" onClick={handleSubmit.bind(this, 'market_order')}
                        loading={placeOrder.submitButtonLoading}>{intl.get('actions.place_sell_order')}</Button>
              }
            </div>
          </div>
        </div>
  ...
  render() {
    ...
    const btns = (
      <div style={{width: '180px'}}>
        <Button onClick={gotoTransfer.bind(this, item)}
                className="d-block w-100 text-left mb5">Send {item.symbol}</Button>
        <Button onClick={gotoReceive.bind(this, {symbol: item.symbol})}
                className="d-block w-100 text-left mb5">Receive {item.symbol}</Button>
        {
          item.symbol === 'WETH' &&
          <Button onClick={gotoConvert.bind(this, {symbol: "WETH"})} className="d-block w-100 text-left mb5">Convert
            WETH To ETH</Button>
        }
        {
          item.symbol === 'WETH' &&
          <Button onClick={gotoConvert.bind(this, {symbol: "ETH"})} className="d-block w-100 text-left mb5">Convert ETH
            To WETH</Button>
        }
      </div>
    )
    return (
      <div className="more token-action" onClick={e => {
        e.stopPropagation();
        e.preventDefault()
      }}>
        <Popover
