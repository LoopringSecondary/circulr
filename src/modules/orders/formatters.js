  ...
  if (balanceS.balance.lt(tradeInfo.amountS)) {
    error.push({
      ...
      value: {
        ...
        required: ceilDecimal(tradeInfo.amountS.minus(balanceS.balance), 6)
      }
    })
    failed = true
  }
  ...
  if (ethBalance.balance.lt(gas)) {
    error.push({
      ...
      value: {
        ...
        required: ceilDecimal(gas.minus(ethBalance), 6)
      }
    })
    failed = true
  }
  ...
  if (approveWarn && approveWarn.length > 0) {
    ...
    approveWarn.forEach(item => {
      ...
      const approve = generateApproveTx({
        ...
        gasPrice,
        gasLimit,
        amount: fm.toHex(fm.toBig('9223372036854775806').times('1e' + tokenConfig.digits || 18)),
        nonce: fm.toHex(nonce)
      })
      unsigned.push({
        type: 'tx',
        data: approve,
        token: item.value.symbol,
        index: (item.value.allowance > 0 ? 1 : 0),
        action: 'ApproveAllowance',
        address: wallet.address
      })
      nonce = nonce + 1;
    });
...
export function getMarketPrice(items, token, mToken) {
  ...
  if (tPrice && mtPrice) {
    return toNumber(toFixed(tPrice.price / mtPrice.price,market && market.pricePrecision && 6))
  }
  return 0
}
