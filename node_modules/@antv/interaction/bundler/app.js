      ...
      if (req.method === 'GET') {
        ...
        if (pathname === '/bundle') {
          ...
          webpack(config, (err, stats) => {
            ...
            // step3: uglify
            shelljs.exec('uglifyjs -c -m -o __dist/interaction.min.js -- __dist/interaction.js');
            // step4: zipping it
            const zip = new JSZip();
            zip.folder('interaction-dist').file('entry.js', entryFileContent);
            zip.folder('interaction-dist').file('interaction.js', readFileSync(join(distPath, './interaction.js'), 'utf8'));
            zip.folder('interaction-dist').file('interaction.js.map', readFileSync(join(distPath, './interaction.js.map'), 'utf8'));
            zip.folder('interaction-dist').file('interaction.min.js', readFileSync(join(distPath, './interaction.min.js'), 'utf8'));
            res.writeHead(200, {
